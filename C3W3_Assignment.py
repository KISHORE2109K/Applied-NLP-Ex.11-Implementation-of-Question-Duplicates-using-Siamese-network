...
def Siamese(text_vectorizer, vocab_size=36224, d_feature=128):
    ...
    Args:
        ...
        vocab_size (int, optional): Length of the vocabulary.
    ...
    # LSTM layer with sequences returned
...
def TripletLossFn(v1, v2, margin=0.25):
    ...
    # subtract the diagonal from scores
    negative_zero_on_duplicate = scores - tf.linalg.diag(positive)

    # use `tf.math.reduce_sum` on `negative_zero_on_duplicate` and divide it by `(batch_size - 1)`
    mean_negative = tf.math.reduce_sum(negative_zero_on_duplicate, axis=1) / (batch_size - 1)

    # create mask to exclude positive examples
    mask_exclude_positives = tf.cast(
        (tf.eye(tf.shape(v1)[0], dtype=tf.bool) |
         (negative_zero_on_duplicate > tf.expand_dims(positive, 1))),
        scores.dtype
    )

    # subtract out those masked values
    negative_without_positive = negative_zero_on_duplicate - (mask_exclude_positives * 2.0)

    # take the row by row max - closest negative
...
def predict(question1, question2, threshold, model, verbose=False):
    ...
    # Split the output into the two encodings
    v1 = v1v2[:, :v1v2.shape[1] // 2]
    v2 = v1v2[:, v1v2.shape[1] // 2:]

    # Cosine similarity via element-wise product + sum
    d = tf.math.reduce_sum(v1 * v2, axis=1)
    ...
    if verbose:
        print("Q1  =", question1)
        print("Q2  =", question2)
        print("d   =", float(d.numpy()))
        print("res =", bool(res.numpy()))

    return bool(res.numpy())
